...
const MovieList: React.FC<Props> = ({ groupId, initialType = 'watchlist' }) => {
  ...
  const fetchUserPreferences = async (movieEntries: MovieEntry[]) => {
    ...
    try {
      ...
      await Promise.all(
        movieIds.map(async movieId => {
          const voteRef = doc(
            FIRESTORE_DB,
            'movieVotes',
            `${currentUser.uid}_${movieId}_${groupId}`
          );
          const voteSnap = await getDoc(voteRef);
          votesMap[movieId] = voteSnap.exists()
          ...
          const ratingRef = doc(
            FIRESTORE_DB,
            'userRatings',
            `${currentUser.uid}_${movieId}`
          );
          const ratingSnap = await getDoc(ratingRef);
          ratingsMap[movieId] = ratingSnap.exists()
      ...
      return movieEntries.map(entry => {
        ...
        const userHasWatched = !!(
          currentUser && watchedBy.includes(currentUser.uid)
        );
        return {
          ...entry,
  ...
  const fetchMovieDetails = async (movieEntries: MovieEntry[]) => {
    ...
    voteSnapshots.forEach(snap => {
      const { movieId, vote } = snap.data() as any;
        ...
        try {
          ...
          if (!username) {
            const userSnap = await getDoc(
              doc(FIRESTORE_DB, 'users', entry.addedBy)
            );
          ...
          const rottenTomatoesRating =
            details.Ratings?.find(r => r.Source === 'Rotten Tomatoes')
              ?.Value || '';
          const numericRating = rottenTomatoesRating
          ...
          return {
            ...
            username,
            thumbsUp: (voteCounts[entry.imdbID]?.up || 0),
            thumbsDown: (voteCounts[entry.imdbID]?.down || 0),
            userVote: entry.userVote ?? null,
            seen: entry.seen ?? false,
            rottenTomatoesRating: numericRating,
            userRating: averageRating,
            watchedCount: entry.watchedCount ?? 0,
            groupMemberCount: entry.groupMemberCount ?? 0,
            userHasWatched: entry.userHasWatched ?? false,
          };
        } catch (err) {
    ...
    const sortedMovies: MovieWithDetails[] =
      sortBy === 'Liked'
        ? detailedMovies.sort(
            (a, b) =>
        ...
        ? detailedMovies.sort(
            (a, b) =>
              (b.rottenTomatoesRating || 0) -
              (a.rottenTomatoesRating || 0)
          )
        : detailedMovies.sort(
            (a, b) =>
              a.addedAt?.toDate?.().getTime() -
              b.addedAt?.toDate?.().getTime()
          );
  ...
  const filterMoviesByActiveTab = (moviesList = allMovies) => {
    ...
    const watchedMovies = moviesList.filter(m => m.seen);
    let filtered = moviesList.filter(m =>
      activeTab === 'watchlist' ? !m.seen : m.seen
    );

    // fairness filter: hide culprit’s movies
    if (
      fairnessFilter &&
      activeTab === 'watchlist' &&
  ...
  const removeMovie = async (imdbID: string) => {
    try {
      ...
      const currentMovies: MovieEntry[] =
        movieDoc.data()?.movies || [];
  ...
  const castVote = async (imdbID: string, vote: 'up' | 'down') => {
    ...
    try {
      ...
      if (movie.userVote === vote) {
        newVote = null;
        vote === 'up' ? (thumbsUpDelta = -1) : (thumbsDownDelta = -1);
      } else {
      ...
      const filteredIdx = movies.findIndex(
        m => m.imdbID === imdbID
      );
      if (filteredIdx !== -1) {
        const newMovies = [...movies];
        newMovies[filteredIdx] = updatedMovie;
        setMovies(newMovies);
      }
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.movieCard}>
              <TouchableOpacity
                onPress={() =>
                  router.push(`/MovieDetails/${item.imdbID}/${groupId}`)
                }
              >
                <Image
                  ...
                  <View style={styles.votesContainer}>
                    ...
                    <View style={styles.seenContainer}>
                      ...
                      >
                        <Image
                          source={
                            item.userHasWatched ? seenIcon : notSeenIcon
                          }
                          style={styles.voteIcon}
                        />
                      </TouchableOpacity>
